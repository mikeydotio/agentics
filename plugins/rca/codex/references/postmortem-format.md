...
project, written by `technical-writer` via runtime.md, plus a condensed comment on the latched issue.
...
## Lessons
<What this taught about the codebase. Candidates for AGENTS.md gotchas.>
```
...
## Lesson offer (one native question)

**AGENTS.md gotcha** — when the lesson is a durable constraint an agent would otherwise
re-trip on ("X must be validated at Y", "never call A before B"). Offer the exact one-line
addition; the user approves before any AGENTS.md edit.
