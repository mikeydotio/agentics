...
mkdir -p "${PROJECT_DIR}/.pilot/handoffs"
HANDOFF_FILE="${PROJECT_DIR}/.pilot/handoffs/handoff-execute.md"
...
(patterns, micro-decisions, code landmarks) that a graceful /pilot stop would include.
- **source**: stop-hook
...
if [[ -n "${TMUX:-}" ]] && [[ -n "${TMUX_PANE:-}" ]]; then
  ...
  for existing in "$FRESHEN_DIR"/*.signal; do
    ...
    if [[ "$existing_source" != "pilot" ]]; then
      CONFLICT=1
      break
    fi
  done
  if [[ "$CONFLICT" -eq 0 ]]; then
    echo "/pilot resume" > "$FRESHEN_DIR/pilot.signal"
  fi
fi
