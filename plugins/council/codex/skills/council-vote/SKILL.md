...
- Read `<plugin-root>/references/council-protocol.md` for prompt templates, response schemas,
  artifact formats, voting phases, and response-failure policy.
- Read `<plugin-root>/references/liveness.md` completely and use
  `<plugin-root>/bin/council-state.py` for all four dispatch phases and recovery.
...
4. Members cannot modify repository files. Measurement artifacts belong only in each
   assigned private scratch directory; canonical metadata does not relax this boundary.
5. Persist the phase artifacts before advancing. Never overwrite an earlier council directory.
6. Missing, malformed, blocked, and failed output gets exactly one retry, then abstention.
   The helper bounds research at 1500 seconds and every later phase at 300 seconds.
   Two abstentions in one phase write `ABORT.md` and end without a decision.
   Surface retries, extensions, abstentions, and aborts; never wait indefinitely for user input.
