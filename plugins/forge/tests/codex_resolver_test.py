...
class AgentResolution(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.temp = tempfile.TemporaryDirectory(prefix="forge resolver ", dir="/private/tmp")
        self.addCleanup(self.temp.cleanup)
        self.root = Path(self.temp.name)
        self.plugin = self.root / "market/forge/3.9.1"
        self.plugin.joinpath("bin").mkdir(parents=True)
        shutil.copy(ROOT / "bin/resolve-dependency.sh", self.plugin / "bin")
    ...
    def run_resolver(self, expected=0):
        ...
        result = subprocess.run(["bash", str(self.plugin / "bin/resolve-dependency.sh"), "agents"],
    ...
    def test_role_resolution_and_missing_role(self):
        ...
        for role in ("domain-researcher", "generator", "evaluator", "reviewer", "validator", "triager",
                     "qa-engineer", "project-manager", "skeptic", "software-architect", "technical-writer"):
    ...
    def test_legacy_installed_dependency(self):
        """Codex installs legacy Claude manifests; identity/version must still match."""
        root = self.agents(self.root / "legacy")
        (root / ".codex-plugin").rename(root / ".claude-plugin")
        self.registered(root)
        self.assertEqual(self.run_resolver(), str(root))
