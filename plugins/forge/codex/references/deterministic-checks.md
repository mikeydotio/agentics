...
```bash
bash "<plugin-root>/bin/forge-prechecks.sh" --story-id <id> --mapping .forge/plan-mapping.json
```
...
- `all_passed: true` → proceed to the evaluator (Step 5 of `<plugin-root>/codex/references/execution-loop.md`).
- `all_passed: false` → find the failing check(s) in `checks[]`, store its `details` as a storyhook
  comment, and go to retry (`<plugin-root>/codex/references/execution-loop-retry.md`) — do not invoke the
