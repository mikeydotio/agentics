...
> **Project standards, environment, and git policy.** Everything from here to the
> `BEGIN GENERATED` marker is hand-maintained and outside the generated fence —
> `story scaffold agents-md` only adds or repairs the fenced block below it.
>
> `CLAUDE.md` is a symlink to this file, so Claude Code auto-loads the same text.
