...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/bin/rca-worktree.sh create <slug> --copy <repro-test-path> --setup-cmd "<deps cmd if needed>"
bash ${CLAUDE_PLUGIN_ROOT}/bin/rca-bisect.sh run <slug> --good <ref> --bad HEAD --test-cmd "<single-test cmd>"
```
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/bin/rca-forensics.sh pickaxe --term <symbol-or-string> --since <date>
```
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/bin/rca-forensics.sh timeline --paths <implicated paths> --since <boundary>
bash ${CLAUDE_PLUGIN_ROOT}/bin/rca-hotspots.sh --paths <implicated dirs> --since 12.months
```
