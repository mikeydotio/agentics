...
Complete specification for the autonomous execution loop. The SKILL.md router dispatches here for `$forge:forge run` and `$forge:forge resume`.
...
- `<plugin-root>/codex/references/execution-loop-complete.md` — **only when Step 1's `story next` reports every
...
1. Generated a session ID and acquired the session lock: `bash
   "<plugin-root>/bin/forge-lock.sh" acquire --session-id "$SESSION_ID" --forge-dir .forge`
   (see `<plugin-root>/codex/references/session-locking.md`) — if `acquired` is `false`, STOP here.
...
```bash
bash "<plugin-root>/bin/forge-loop-state.sh" runaway-check --forge-dir .forge
```
...
Parse the response:
- **Story returned**: `bash "<plugin-root>/bin/forge-loop-state.sh" storyhook-failure --result ok --forge-dir .forge`
  (resets the persisted consecutive-failure counter), proceed with this story
- **No story, all done**: `bash "<plugin-root>/bin/forge-loop-state.sh" storyhook-failure --result ok --forge-dir
  .forge`, `goto complete`
- **No story, some blocked**: `bash "<plugin-root>/bin/forge-loop-state.sh" storyhook-failure --result ok --forge-dir
  .forge`, `write_handoff("blocked stories remain — user intervention needed")`, `goto pause`
- **Storyhook error** (nonzero exit / unparseable JSON): `bash "<plugin-root>/bin/forge-loop-state.sh"
...
4. **Predecessor diffs**:
   ```bash
   bash "<plugin-root>/bin/forge-predecessor-diff.sh" --limit-stories 3 --limit-lines 5000 --project-dir .
   ```
...
bash "<plugin-root>/bin/forge-lock.sh" heartbeat --session-id "$SESSION_ID" --forge-dir .forge
# Require the clean generator baseline described in runtime.md before spawning.
bash "<plugin-root>/bin/forge-integrity.sh" snapshot --phase pre-gen --forge-dir .forge --scope forge-only --session-id "$SESSION_ID"
```

Resolve the generator canonical role and Forge override through runtime.md. Spawn a separate generator:

```
Dispatch task specification (
  canonical role: "generator",
  prompt: <constructed prompt with:
    ...
    - Generator agent instructions (from generator.md) + forge override
      (from <plugin-root>/codex/agent-overrides/generator-context.md)
  >
)
```
...
```bash
bash "<plugin-root>/bin/forge-integrity.sh" check --phase pre-gen --forge-dir .forge --scope forge-only --session-id "$SESSION_ID"
```
...
```bash
bash "<plugin-root>/bin/forge-prechecks.sh" --story-id HP-N --mapping .forge/plan-mapping.json
```
...
bash "<plugin-root>/bin/forge-lock.sh" heartbeat --session-id "$SESSION_ID" --forge-dir .forge
bash "<plugin-root>/bin/forge-integrity.sh" snapshot --phase pre-eval --forge-dir .forge --scope full-tree --session-id "$SESSION_ID"
```

Resolve the evaluator canonical role and Forge override through runtime.md. Spawn an independent evaluator; enforce the full-tree integrity gate before using its verdict:

```
Dispatch task specification (
  canonical role: "evaluator",
  prompt: <constructed prompt with:
    ...
    - Evaluator agent instructions (from evaluator.md) + forge override
      (from <plugin-root>/codex/agent-overrides/evaluator-context.md)
  >
)
```
...
```bash
bash "<plugin-root>/bin/forge-integrity.sh" check --phase pre-eval --forge-dir .forge --scope full-tree --session-id "$SESSION_ID"
```

The full-tree integrity check is mandatory on Codex. Native children inherit runtime
permissions; a role definition is not a tool-access barrier. If snapshot or check returns
ok: false, halt with an incomplete handoff before acting on a verdict.
...
```bash
bash "<plugin-root>/bin/forge-verdict.sh" --story HP-N --attempt <attempt_number> \
...
```bash
bash "<plugin-root>/bin/forge-loop-state.sh" attempt --forge-dir .forge
bash "<plugin-root>/bin/forge-lock.sh" heartbeat --session-id "$SESSION_ID" --forge-dir .forge
```
...
```bash
bash "<plugin-root>/bin/forge-loop-state.sh" done --forge-dir .forge
```
...
```bash
bash "<plugin-root>/bin/forge-loop-state.sh" architect-check --wave-boundary true|false --forge-dir .forge
```
...
- `trigger: true` → run the architect review:
  ...
  Resolve software-architect and its Forge override through runtime.md.

  ```
  Dispatch task specification (
    canonical role: "software-architect",
    prompt: <constructed prompt with:
      - software-architect.md + <plugin-root>/codex/agent-overrides/software-architect-context.md
...
nowhere else. Specified in `<plugin-root>/codex/references/execution-loop-retry.md`: read it at the moment an
...
```
pause:
  # The handoff MUST include Cold-Start Essentials (see <plugin-root>/codex/references/handoff-format.md):
...
`$forge:forge resume` (or reports `fallback_message` if tmux is unavailable — not a fatal error, pause
completes normally either way):

```bash
bash "<plugin-root>/bin/forge-step-exit.sh" --host codex --step execute \
  --summary "paused — ${STORIES_THIS_SESSION} stories completed. ${REASON}" --next '$forge:forge resume'
```

### Complete

`goto complete` — reached from Step 1 when `story next` reports every story done, and from
nowhere else. Specified in `<plugin-root>/codex/references/execution-loop-complete.md`: the final test suite, the
