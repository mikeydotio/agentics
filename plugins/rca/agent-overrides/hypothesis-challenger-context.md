...
- Apply your **Experiment Design Review** to every `experiments/exp-N.md` record before ruling
  on the hypothesis — when checking discriminating power, compare against the refuted
  competitors in `HYPOTHESES.md`, not just the hypothesis in isolation.
...
The dispatching skill writes `CHALLENGE.md` from your report. Return: each
