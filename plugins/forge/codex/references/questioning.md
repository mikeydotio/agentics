...
Before dispatching research, follow `<plugin-root>/references/delivery.md` for durable pending state,
...
## Question Format (the native question tool)

**All questions to the user MUST use the `the native question tool` tool** with exactly 1 question per call. This mechanically enforces one-question-at-a-time and structures responses for easier decision-making.
...
After 4 the native question tool calls on a topic:
- header: "Direction"
...
4. Resume questioning with the native question tool, incorporating findings into the next question

Never call the native question tool while research is pending.
