...
`.forge/TRIAGE.md` yourself. The orchestrator (`<plugin-root>/codex/skills/triage/SKILL.md`) synthesizes that file by
combining your decisions with the qa-engineer's and skeptic's perspectives, then routes FIX
decisions to a generator agent and ESCALATE decisions to the user via `the native question tool`.
