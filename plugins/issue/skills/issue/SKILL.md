...
- **GitHub write-backs live in the helper — never call `gh`/`git` yourself.** `do`'s default prompt
  ...
  (`Closes #<n>`), comment each PR link, and **never bump the version or deploy from the worktree**
  (versioning/deployment happen later from `main`; the semver/deployit CLIs also hard-refuse inside a
  worktree). `complete`'s cleanup (close, worktree/branch removal) and `new`'s filing are likewise the
  helper's job.
