...
priorities, and acceptance criteria. See `<plugin-root>/references/storyhook-contract.md` for the full command
...
- **If exists**: Compute MD5 hash of PLAN.md and compare against `plan_hash` in the mapping
  - **Hash matches**: Offer three options via the native question tool:
...
2. The execution loop's Complete step (`<plugin-root>/codex/references/execution-loop-complete.md`) additionally closes it for
