...
(`<plugin-root>/codex/references/step-handoff.md`) — write `handoff-triage.md` (Triage Handoff table) and run:
```bash
bash "<plugin-root>/bin/forge-step-exit.sh" --host codex --step triage \
  --summary "[FIX count] FIX, [ESCALATE count] ESCALATE" --next '$forge:forge continue'
```
No `--extra-path` is needed: any ESCALATE stories this step created live in storyhook's own
store outside the repository (see `<plugin-root>/codex/references/handoff-format.md`).
