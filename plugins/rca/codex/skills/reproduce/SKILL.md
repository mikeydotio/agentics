...
You are running the reproduction gate. Read `<plugin-root>/codex/references/repro-gate.md`
first. Inputs: `.rca/<slug>/GRID.md` + `meta.json` (resolve the slug from the argument, or via
`bash "<plugin-root>/bin/rca-status.sh"` when exactly one investigation is at
...
```bash
bash "<plugin-root>/bin/rca-stack.sh" detect
```
- One clear primary → confirm silently, record.
- Ambiguous (multi-stack, `needs:["scheme"]`, or `none`) → one native question (options from
...
`runtime.md`: use native `spawn_agent` for qa-engineer per runtime.md; prompt = contents of
`<plugin-root>/agent-overrides/qa-engineer-context.md` + dynamic context: GRID.md
...
1. `git status --porcelain` in the main tree: nothing beyond `.rca/`, the declared new test
   file(s), and the scaffold's `.gitignore` change relative to the pre-dispatch baseline.
   Unexpected changes → halt and preserve diagnostics per runtime.md; never blindly revert.
2. **Fails for the right reason**:
   ```bash
   bash "<plugin-root>/bin/rca-repro.sh" run --cmd "<single-test cmd>" --runs 1
   ```
...
Present the honest rationale (every attempt + failure reason). Then one native question:
...
`tier_directive` is set in meta.json, apply it WITHOUT asking. Otherwise one native question
...
standalone). State is fully on disk — safe to start a fresh session.
