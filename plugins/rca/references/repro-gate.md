...
Spawn `agents:qa-engineer` in Reproduction Mode with: GRID.md, the stack's test command info,
...
Verify with:
```bash
bash ${CLAUDE_PLUGIN_ROOT}/bin/rca-repro.sh run --cmd "<single-test command>" --runs 1
```
...
candidate (LIGHT). One AskUserQuestion to confirm — skipped entirely when the user invoked
