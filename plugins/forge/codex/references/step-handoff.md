...
**The handoff is mandatory, not best-effort.** After `/new`, it is the only source of session knowledge. If the handoff is missing on resumption, the orchestrator MUST pause and ask the user how to proceed via `the native question tool` rather than continuing with degraded context.
...
3. **Run the step-exit script** — this single call replaces hand-rolled `git commit` +
   ...
   ```bash
   bash "<plugin-root>/bin/forge-step-exit.sh" --host codex --step <step> --summary "<summary>" --next "<next-command>" --transition-id "<transition-id>"
   ```
   - `--next` — the specific next step command when the transition is deterministic (e.g.,
     `$forge:forge research --orchestrated` after interrogate), or `$forge:forge continue` when the next step
   ...
   - `--transition-id <id>` (agentics#33, optional) — the `transition_id` from this turn's
     `forge-state.sh --record-transition` JSON output (see `<plugin-root>/codex/skills/forge/SKILL.md`'s State
   ...
   - `--extra-path <path>` (repeatable) — stage additional paths beyond `.forge/` in the same
     ...
     repository, so no step commits it (see `<plugin-root>/codex/references/handoff-format.md`).
   - The script itself resolves its own location via `SCRIPT_DIR`, so `bash
     "<plugin-root>/bin/forge-step-exit.sh" --host codex` is the only invocation form to use — never a
   ...
   - Parse the returned JSON: if `freshen_queued` is `false`, show the user the `fallback_message`
     (manual `/new` + next-command instructions — no tmux available). `committed: false` is a
...
On "Continue anyway", see `<plugin-root>/codex/references/handoff-format.md`'s **Recovery Without Handoff** for what
...
After deleting, re-invoke `$forge:forge continue` — the state detection picks up from the correct step.
