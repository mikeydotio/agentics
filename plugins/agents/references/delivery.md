...
| progress | agent_id, working, evidence | Current-attempt progress, not presence or idle notification |
...
Terminal state is durable before cleanup. Cleanup uses a separate monotonic anchor
and gets 30 seconds total; interrupt only
...
the batch. The system CLOCK_MONOTONIC clock governs elapsed waits across Python processes; backward clocks or more than five seconds
