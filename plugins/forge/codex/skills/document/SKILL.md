...
- `<plugin-root>/codex/references/team-roles.md` (before spawning — "Resolving canonical role" governs the
...
Resolve `canonical role` per `<plugin-root>/codex/references/team-roles.md` (load the canonical role and Forge override through runtime.md).
...
Present documentation sections to the user as **plain text**. For key sections, use the native question tool:
...
the Step Exit Protocol (`<plugin-root>/codex/references/step-handoff.md`) — write `handoff-document.md` (Document
Handoff table) and run:
```bash
bash "<plugin-root>/bin/forge-step-exit.sh" --host codex --step document \
  --summary "project documentation" --next '$forge:forge continue'
```
