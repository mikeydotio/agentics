...
| macOS deploy produced no appcast / `.zip` (deploy still succeeded) | `[macos.sparkle] enabled = false`, or this was a `--no-release`/`[github] release = false` deploy where `sign_update` / the key wasn't found (deploy logs a "skipping Sparkle" warning naming the resolved binary) | Set `enabled = true` and a valid `sign_update_path` + `private_key_path` in `config.toml`; check the deploy stderr / `backend.err.log` for the skip reason, then re-deploy. |
| macOS deploy FAILS outright with "refusing to ship an unsigned release" | `[macos.sparkle] enabled = true` and this deploy publishes a GitHub release (the default), but `sign_update` couldn't sign — issue #117: an unsigned Developer-ID release is a failed release, so deployit refuses to publish it rather than shipping it silently unsigned. Nothing was staged (tailnet index, backend, and release are all untouched). | The failure message names the resolved binary and, if it was a silent Gatekeeper kill (non-zero exit, empty stdout/stderr), the fix: `xattr -d com.apple.quarantine <path>`. Fix the toolchain (see `references/sparkle.md` §3), then re-deploy. To ship anyway without Sparkle for this one deploy, pass `--no-release` (tailnet only, degrades gracefully) or temporarily set `[macos.sparkle] enabled = false`. |
