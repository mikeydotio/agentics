...
2. **RED.** `rca-repro.sh run --cmd "<repro cmd>" --runs 1` → must still FAIL. A passing repro
   ...
   test proves nothing.
3. **Implement — hat #1.** Spawn `agents:software-engineer` briefed with DIAGNOSIS.md +
...
7. **Sibling-pattern sweep** (fix-it-everywhere). Derive a search signature from the ODC
   ...
   experimenter toggled). Spawn `agents:evidence-collector` to find other occurrences.
