...
Checks that only matter when routing a **bare idea description or explicit `$forge:forge interrogate`
...
file entirely; `<plugin-root>/codex/skills/forge/SKILL.md`'s Command Router only points here on the interrogate-routing path
...
1. Use the native question tool:
   - **header:** "Legacy Data"
...
2. Run state detection: `bash "<plugin-root>/bin/forge-state.sh"`
...
### Summarize (plain text, not the native question tool)
...
### Offer Options (the native question tool)
...
- **options:**
  ...
  - "Cancel — handle previous work first" / "Abort the new idea so you can resume or finish the existing pipeline. Pros: no data loss, continue where you left off. Cons: delays starting the new idea. Hint: use `$forge:forge continue` to resume."

### Execute User's Choice

- **If "Archive and start fresh":**
  ```bash
  bash "<plugin-root>/bin/forge-archive.sh" .forge .forge-archives
  ```
...
- **If "Cancel":**
  Report: "New idea cancelled. Run `$forge:forge continue` to resume the existing pipeline, or `$forge:forge status` to see where it left off."
  **STOP** — do not proceed.
