...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/bin/forge-lock.sh acquire   --session-id <id> [--window-min <n>] [--forge-dir .forge]
bash ${CLAUDE_PLUGIN_ROOT}/bin/forge-lock.sh heartbeat --session-id <id> [--forge-dir .forge]
bash ${CLAUDE_PLUGIN_ROOT}/bin/forge-lock.sh release   [--session-id <id>] [--forge-dir .forge]
bash ${CLAUDE_PLUGIN_ROOT}/bin/forge-lock.sh check     --session-id <id> [--window-min <n>] [--forge-dir .forge]
```
...
for callers that need to *inspect* lock state without side effects (e.g. `/forge status`-style
tooling).
...
a user manually runs `/forge resume` while a freshen-triggered resume is starting, `acquire`
...
the user must manually run `/forge resume`. The recovery sequence (`references/recovery-protocol.md`,
via `bin/forge-crash-recover.sh`) handles in-progress/verifying stories (resets to todo).
